...
class OtpVerifySignupView extends StatelessWidget {
  const OtpVerifySignupView({super.key});

  @override
  Widget build(BuildContext context) {
    return BlocConsumer<OtpVerifySignupBloc, VerifyOtpState>(
      ...
      listener: (context, state) {
        ...
        if (state.verifyState is UiSuccess) {
          ...
          context.read<OtpVerifySignupBloc>().add(OnResetVerifyOtp());
        }
      ...
      builder: (context, state) {
        return OtpVerifyView(
          ...
          onPinComplete: (String v) {
            context.read<OtpVerifySignupBloc>().add(OnRequestVerifyOtp(otp: v));
          },
          controller: state.controller,
          onResent: () {
            context.read<OtpVerifySignupBloc>().add(OnResendVerifyOtp());
          },
        );
      },
    );
